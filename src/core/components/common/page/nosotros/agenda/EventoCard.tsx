import { HiClock } from 'react-icons/hi';


interface EventoCardProps {
	
	index: number;
	onSelect: (id: string) => void;
}

const EventoCard = ({  index, onSelect }: EventoCardProps) => {
	return (
		<div
			className='border border-blue-100 rounded overflow-hidden cursor-pointer transition-all mb-4 hover:shadow-lg hover:scale-105'
			style={{ animationDelay: `${index * 0.1}s` }}
			>
			<div className='flex'>
				<div className={`w-2 `}></div>
				<div className='flex gap-3 p-3'>
					<div className='flex-shrink-0 p-2 flex flex-col items-center justify-center h-14 w-14 rounded border border-blue-200 bg-blue-50'>
						<span className='text-xs text-blue-400 uppercase'>Jueves</span>
						<span className='text-xl font-bold text-blue-700'>25</span>
					</div>
					<div className='flex-1 min-w-0'>
						<h4 className='font-medium whitespace-nowrap overflow-hidden text-ellipsis'>Día de la bandera</h4>
						
							<span className={` text-xs px-2 py-0.5 rounded-full inline-block mt-1`}>
								Musica
							</span>
						
						<div className='flex items-center text-sm text-gray-500 mt-1'>
							<HiClock className='w-3 h-3 mr-1' />
							7:00 PM - 12:00 PM
						</div>
						<div className='text-sm text-gray-500 whitespace-nowrap overflow-hidden text-ellipsis'>
							Municipalidad de nuevo Chimbote
						</div>
					</div>
				</div>
			</div>
		</div>
	);
};

export default EventoCard;
