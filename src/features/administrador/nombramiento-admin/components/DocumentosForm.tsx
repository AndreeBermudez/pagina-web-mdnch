...
import { obtenerDocumentosNombramiento } from '../services/obtenerDocumentosNombramiento';
...
export const DocumentosForm = ({ convocatoriaId, codigoConvocatoria, nombreConvocatoria, area, handleClose }: DocumentosFormProps) => {
  ...
  useEffect(() => {
    ...
    const loadDocumentos = async () => {
      try {
        const documentos = await obtenerDocumentosNombramiento(convocatoriaId);
        if (!activo) {
          return;
        }

        console.log('📦 Documentos recibidos del backend:', JSON.stringify(documentos, null, 2));
        ...
        console.log('🔄 Documentos procesados:', JSON.stringify(remoteDocumentos, null, 2));

        const gruposMapeados = mapRemoteDocumentosToState(remoteDocumentos);
        console.log('📊 Grupos mapeados:', JSON.stringify(gruposMapeados, null, 2));

        setGrupos(gruposMapeados);
      } catch (error) {
        console.error('Error al obtener documentos del nombramiento:', error);
      }
    };
  ...
  const handleSaveAll = async () => {
    try {
      ...
      const response = await configurarDocumentosNombramiento(convocatoriaId, documentosParaGuardar);
  ...
  const handleToggle = async (grupoId: string, itemId: string) => {
    ...
    if (!itemActual) {
      console.error('Item no encontrado:', itemId, 'en grupo:', grupoId);
      return;
    }
    ...
    try {
      ...
      const response = await configurarDocumentosNombramiento(convocatoriaId, payload);
      ...
      if (response?.documentos) {
        ...
        console.log('🔄 Documentos mapeados desde backend:', JSON.stringify(remoteDocumentos, null, 2));
        setGrupos(mapRemoteDocumentosToState(remoteDocumentos));
      }
    } catch (error) {
