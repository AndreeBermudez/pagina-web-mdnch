...
import type { NombramientoUpdatePayload } from '../services/types';
...
const mapTipoToFormField = (tipo: DocumentoTipo): keyof NombramientoUpdatePayload => {
  switch (tipo) {
...
export const useDocumento = (documento: DocumentoUI): UseDocumentoReturn => {
  ...
  const guardarDocumento = async (documentoUI: DocumentoUI, convocatoriaId: number | string) => {
    ...
    if (archivo || (tipoDocumento === 'POSTULACION' && documentoUI.url)) {
      ...
      const updatePayload: NombramientoUpdatePayload = {};
      ...
      const response = await actualizarNombramiento(idNumerico, updatePayload);
      queryClient.invalidateQueries({ queryKey: ['nombramientos', convocatoriaId] });
    ...
    const documentosActuales = await obtenerDocumentosNombramiento(idNumerico);
    ...
    const response = await configurarDocumentosNombramiento(
      idNumerico,
      payload,
    );
