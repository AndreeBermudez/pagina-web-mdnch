...
const AgendaAdmin = () => {
    ...
    const [agendaToDelete, setAgendaToDelete] = useState<AgendaResponse | null>(null);
    ...
    const confirmDelete = () => {
        if (agendaToDelete) {
            eliminarAgenda.mutate(agendaToDelete.agendaId, {
                onSuccess: () => {
    ...
    return (
        <>
            {isConfirmOpen && agendaToDelete && (
                <ConfirmModal
                    isOpen={isConfirmOpen}
                    onClose={() => setIsConfirmOpen(false)}
                    onConfirm={confirmDelete}
                    title="Eliminar registro"
                    message={`¿Estás seguro de que deseas eliminar el evento "${agendaToDelete.titulo}"?`}
